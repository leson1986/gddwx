<?php
    ...
    $plist->Execute("select C.class_name,P.* from `{$tablepre}products` as P,`{$tablepre}products_class` as C where C.id = P.products_classid and P.`id` = {$id} and `products_isvalid` = 1 order by P.`products_pubtime`");
	$plist->TplParse();
    ...
    $tpl->display("products_detail_cn.html");
