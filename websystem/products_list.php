<?php
    ...
    if (!empty($id)) {
        switch ($action) {
            case "del" : $db->query("delete from `{$tablepre}products` where id in ({$id})"); break;
            case "confirm" : $db->query("update`{$tablepre}products` set `products_isvalid`=1 where id in ({$id})"); break;
            case "deconfirm" : $db->query("update`{$tablepre}products` set `products_isvalid`=0 where id in ({$id})"); break;
        }
    }
    ...
    $page_size    = 15;
    $display_page = "products_list.htm";
    ...
    $search_junk  = "and";
    $query_sql    = "select C.class_name,S.* from `{$tablepre}products` as S, `{$tablepre}products_class` as C where S.products_classid=C.id";
