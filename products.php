...
function get_categories_tree($cat_id = 0)
{
    ...
    */
	...
	{
	$str = $classid;
	$sqlclass = "select id from `{$tablepre}products_class` where parent_id={$classid}";
    ...
    $plist->Execute("select C.class_name_en,P.* from `{$tablepre}products` as P,`{$tablepre}products_class` as C where C.id = P.products_classid ".$where." and `products_isvalid` = 1 order by P.`products_index`");
	$plist->TplParse();
	
    $tpl->assign("cat_tree", get_categories_tree(0));         //产品树形分类
	$tpl->assign("menuid",$menuid);
	$tpl->assign("classid",$classid);
	$tpl->assign("products_name",$products_name);
	$tpl->assign("count",$count);
	$tpl->assign("id",$id);
    $tpl->display("products.html");
?>